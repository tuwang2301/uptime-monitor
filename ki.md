...
- **Tiến độ**: Đã hoàn thành Giai đoạn 1 & 2 (Database schema sync + Backend API register + Scoping monitors + Consecutive failure logic). Bắt đầu Giai đoạn 3 (Nâng cấp Frontend UI).
- **Consent Text**: `Đồng ý`
...
- **Schema mới**:
  - `User` liên kết 1-N với `Monitor`
  - `Monitor` có trường `consecutiveFailures` để chống spam và `isActive` để tạm dừng.
...
- [ ] Cập nhật Frontend UI (`App.tsx` & `AuthModal.tsx` thay thế LoginModal)
